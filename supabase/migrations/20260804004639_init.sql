...
ON inmate_names (inmate_id, first_name, middle_name, last_name);
...
BEGIN
    WITH parsed AS (
        SELECT
            ...
            END AS dob,
            CASE
                WHEN booking_time ~ '^\d{4}-\d{2}-\d{2}T' THEN booking_time::TIMESTAMPTZ
                ELSE NULL
            END AS booking_time,
            first_name TEXT,
            middle_name TEXT,
        ...
        WHERE arrest_id IS NOT NULL AND arrest_id ~ '^\d+$'
        AND inmate_id IS NOT NULL AND inmate_id ~ '^\d+$'
    ),
    ins_agencies AS (
    ...
    ins_officers AS (
        INSERT INTO officer_profiles (arresting_officer)
        SELECT DISTINCT arresting_officer FROM parsed WHERE arresting_officer IS NOT NULL
        ON CONFLICT (arresting_officer) DO NOTHING
    ),
    ins_races AS (
        INSERT INTO race_lookup (race)
        SELECT DISTINCT race FROM parsed WHERE race IS NOT NULL
    ...
    ins_profiles AS (
        ...
        ON CONFLICT (inmate_id) DO NOTHING
        RETURNING inmate_id
    ),
    valid_inmates AS (
        SELECT inmate_id FROM ins_profiles
        UNION
        SELECT inmate_id FROM parsed
    ),
    ins_names AS (
        ...
        FROM parsed p
        JOIN valid_inmates vi ON vi.inmate_id = p.inmate_id
    ...
    ins_arrests AS (
        ...
        WHERE p.inmate_id IN (SELECT inmate_id FROM inmate_profiles)
    ...
    SELECT COUNT(*) INTO rows_affected FROM ins_arrests;
